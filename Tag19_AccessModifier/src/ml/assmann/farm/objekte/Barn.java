package ml.assmann.farm.objekte;
...
public class Barn {
	 ...
	 */
	public Barn() {
		cows = new Livestock[10];
		pigs = new Livestock[10];
	}
	...
	private void setLivestockAt(int index, Livestock ls) {
	...
	public Livestock getPigAt(int index) {
		if (index < 0 || index > this.getPigs().length - 1) {
			System.out.println("Wrong pig position");
			return null;
		}
	...
	public Livestock getCowAt(int index) {
		if (index < 0 || index > this.getCows().length - 1) {
			System.out.println("Wrong cow position");
			return null;
		}
