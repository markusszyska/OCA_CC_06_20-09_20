...
public class Lotto {
	...
	public static void main(String[] args) {
		...
		// Aufgabe 2
		System.out.println("Aufgabe 2: Ziehungen fuer ein Jahr generieren");
		int[][] jahresZiehungen = generateJahresziehung();
		for(int i = 0;i<jahresZiehungen.length;i++) {
			System.out.print("\nDie Zahlen fuer Woche " + (i+1) + " sind: ");
			for(int zahl : jahresZiehungen[i]) System.out.print(zahl + " ");
		}
		System.out.println("\n===========\n");
		
		// Aufgabe 3
		System.out.println("Aufgabe 3: Abgefragte Anzahl an Ziehungen generieren");
		Scanner sc = new Scanner(System.in);
		System.out.println("Geben sie eine Anzahl an Ziehungen ein.");
		int anzahl = sc.nextInt();
		int[][] anzahlZiehungen = generateNumZiehungen(anzahl);
		for(int i = 0;i<anzahlZiehungen.length;i++) {
			System.out.print("\nDie Zahlen fuer Ziehung " + (i+1) + " sind: ");
			for(int zahl : anzahlZiehungen[i]) System.out.print(zahl + " ");
		}
		System.out.println("\n===========\n");
	...
	// Erzeugt 52 Ziehungen und speichert sie in einem 2-dim. Array das zurueckgegeben wird
	public static int[][] generateJahresziehung(){
		int[][] jahresZiehung = new int[52][];
		for(int i = 0;i<jahresZiehung.length;i++) {
			jahresZiehung[i] = generateZiehung();
		}
		return jahresZiehung;
	}
	
	// Erzeugt anzahl an Ziehungen und speichert diese in einem 2-dim Aray das zurueckgegeben wird
	public static int[][] generateNumZiehungen(int anzahl){
		int[][] anzahlZiehung = new int[anzahl][];
		for(int i = 0;i<anzahlZiehung.length;i++) {
			anzahlZiehung[i] = generateZiehung();
		}
		return anzahlZiehung;	
	}
}
