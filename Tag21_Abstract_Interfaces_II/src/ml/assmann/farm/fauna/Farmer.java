...
import ml.assmann.farm.objects.*;

public class Farmer extends Omnivore {
	/*
	 * Attributes
	 */
	private int height;
	...
	public void feed(ALivestock ls, IEdible ie) {
		boolean eaten = ls.eat(ie);
		if(eaten) System.out.println(ls.getClass().getSimpleName() + " \"" + ls.getName() + "\" eats a " + ie.getClass().getSimpleName() + " with the size of " + ie.getWeight() + " and now weights " + ls.getWeight() + " kg");
	}
	...
	public void feedAllLivestock(Bucket bucket) {
		...
		for(ALivestock cow : this.getFarm().getBarn().getCows()) {
			if(cow == null) {
				continue;
			}
		...
		for(ALivestock pig : this.getFarm().getBarn().getPigs()) {
			if(pig == null) {
				continue;
			}
			if(currentAmountCarrots == 0 && !carrotStatus) {
